...
export default {
  ...
  computed: {
    zongjine:{
      get: function () {
        return 1*this.ruleForm.yugoushuliang*this.ruleForm.yugoujiage
      }
    },
  },
  created() {
  ...
  methods: {
    ...
    onSubmit() {
      ...
      this.$refs["ruleForm"].validate(valid => {
        if (valid) {
          this.$http({
            url: `yugoudengji/${!this.ruleForm.id ? "save" : "update"}`,
            method: "post",
            data: this.ruleForm
          }).then(({ data }) => {
            if (data && data.code === 0) {
              this.$message({
                ...
                onClose: () => {
                  ...
                  this.parent.yugoudengjiCrossAddOrUpdateFlag = false;
                  this.parent.search();
