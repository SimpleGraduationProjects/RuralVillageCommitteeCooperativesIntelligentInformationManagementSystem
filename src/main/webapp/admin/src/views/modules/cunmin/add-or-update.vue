<template>
  <div class="addEdit-block">
    ...
    >
      ...
      <el-col :span="12">
        <el-form-item class="select" v-if="type!='info'"  label="民族" prop="minzu">
          <el-select v-model="ruleForm.minzu" placeholder="请选择民族">
            <el-option
                v-for="(item,index) in minzuOptions"
                v-bind:key="index"
                :label="item"
                :value="item">
            </el-option>
...
export default {
  data() {
	...
	jiatingqingkuang : false,
      ...
      rules: {
          cunminzhanghao: [
                { required: true, message: '村民账号不能为空', trigger: 'blur' },
          ],
          mima: [
  ...
  methods: {
    ...
    onSubmit() {
      ...
      this.$refs["ruleForm"].validate(valid => {
        if (valid) {
          this.$http({
            url: `cunmin/${!this.ruleForm.id ? "save" : "update"}`,
            method: "post",
            data: this.ruleForm
          }).then(({ data }) => {
            if (data && data.code === 0) {
              this.$message({
                ...
                onClose: () => {
                  ...
                  this.parent.cunminCrossAddOrUpdateFlag = false;
                  this.parent.search();
