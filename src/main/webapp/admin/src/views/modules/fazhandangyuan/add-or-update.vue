<template>
  <div class="addEdit-block">
    ...
    >
      ...
      <el-col :span="12">
        <el-form-item class="input" v-if="type!='info'"  label="介绍人" prop="jieshaoren">
          <el-input v-model="ruleForm.jieshaoren" 
              placeholder="介绍人" clearable  :readonly="ro.jieshaoren"></el-input>
        </el-form-item>
        <div v-else>
          <el-form-item class="input" label="介绍人" prop="jieshaoren">
              <el-input v-model="ruleForm.jieshaoren" 
                placeholder="介绍人" readonly></el-input>
          </el-form-item>
        </div>
      </el-col>
      <el-col :span="12">
      ...
      <el-col :span="12">
        <el-form-item class="date" v-if="type!='info'" label="批准日期" prop="pizhunriqi">
            <el-date-picker
                format="yyyy 年 MM 月 dd 日"
                value-format="yyyy-MM-dd"
                v-model="ruleForm.pizhunriqi" 
                type="date"
                placeholder="批准日期">
            </el-date-picker> 
        </el-form-item>
        <div v-else>
          <el-form-item class="input" v-if="ruleForm.pizhunriqi" label="批准日期" prop="pizhunriqi">
              <el-input v-model="ruleForm.pizhunriqi" 
                placeholder="批准日期" readonly></el-input>
          </el-form-item>
        </div>
      </el-col>
      </el-row>
  ...
  methods: {
    ...
    init(id,type) {
      ...
      if(this.type=='info'||this.type=='else'){
	    ...
	    this.ro.zhengzhimianmao = true;
          ...
          if(o=='pizhunriqi'){
            this.ruleForm.pizhunriqi = obj[o];
	    this.ro.pizhunriqi = true;
            continue;
          }
        }
      }
      ...
      this.$http({
        ...
        if (data && data.code === 0) {
          ...
          this.$message.error(data.msg);
            ...
            this.zhengzhimianmaoOptions = "群众,团员".split(',')
    },
    // 多级联动参数
    info(id) {
      this.$http({
        url: `fazhandangyuan/info/${id}`,
        method: "get"
      }).then(({ data }) => {
        if (data && data.code === 0) {
    ...
    onSubmit() {
      ...
      this.$refs["ruleForm"].validate(valid => {
        if (valid) {
          this.$http({
            url: `fazhandangyuan/${!this.ruleForm.id ? "save" : "update"}`,
            method: "post",
            data: this.ruleForm
          }).then(({ data }) => {
            if (data && data.code === 0) {
              this.$message({
                ...
                onClose: () => {
                  ...
                  this.parent.fazhandangyuanCrossAddOrUpdateFlag = false;
                  this.parent.search();
